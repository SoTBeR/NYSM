...
- **AI-ранжирование** — модель DeepSeek через [gen-api.ru](https://gen-api.ru) переранжирует найденные фильмы под смысл вашего запроса
- **Карточки фильмов** — название, год, режиссёр, жанры, длительность, студия; клик открывает полную информацию
...
>
> Приложение не подписано сертификатом Apple Developer. macOS заблокирует запуск и покажет сообщение «не удаётся открыть, так как разработчик не установлен». Это стандартная ситуация для open-source приложений без платной подписи Apple.
>
> Перед открытием `.dmg` выполните в терминале:
>
> ```bash
> sudo xattr -rd com.apple.quarantine /путь/к/файлу.dmg
> ```
>
> Например:
>
...
Проект распространяется под лицензией [MIT](LICENSE) — используйте, форкайте и модифицируйте свободно.
